...
export const Prefs = {
  async load(): Promise<void> {
    try {
      ...
      const migrated = migrateEnvironmentPrefs(_cache);
      _cache.environments = normalizeEnvironments(migrated.environments);
      _cache.environmentHostGroupRules = sanitizeEnvironmentRules(
        normalizeEnvironmentRules(migrated.rules),
        _cache.environments,
      );
      if (_cache.minSeverityDisplay === undefined) {
        _cache.minSeverityDisplay = Constants.DEFAULT_PREFS.MIN_SEVERITY_DISPLAY;
      }
      if (!_cache.problemTitleHideRules) {
        _cache.problemTitleHideRules = [];
      }
      if (_cache.excludeDisabledHosts === undefined) {
        _cache.excludeDisabledHosts = Constants.DEFAULT_PREFS.EXCLUDE_DISABLED_HOSTS;
      }
      if (_cache.excludeMaintenanceHosts === undefined) {
        _cache.excludeMaintenanceHosts = Constants.DEFAULT_PREFS.EXCLUDE_MAINTENANCE_HOSTS;
      }
      if (_cache.hideAcknowledgedProblems === undefined) {
        _cache.hideAcknowledgedProblems = Constants.DEFAULT_PREFS.HIDE_ACKNOWLEDGED_PROBLEMS;
      }
      if (_cache.excludeDisabledTriggers === undefined) {
        _cache.excludeDisabledTriggers = Constants.DEFAULT_PREFS.EXCLUDE_DISABLED_TRIGGERS;
      }
