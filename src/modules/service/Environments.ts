...
const LEGACY_ENV_LABELS: Record<string, string> = {
...
export function migrateEnvironmentPrefs(stored: Partial<AppPrefs>): {
  environments: EnvironmentDefinition[];
  rules: EnvironmentHostGroupRule[];
} {
  if (stored.environments?.length) {
    const environments = normalizeEnvironments(stored.environments);
    const rules = sanitizeEnvironmentRules(
      normalizeEnvironmentRules(stored.environmentHostGroupRules),
      environments,
    );
    return { environments, rules };
  }

  const customIds = new Set<string>();
  const customEnvs: EnvironmentDefinition[] = [];
  for (const rule of stored.environmentHostGroupRules ?? []) {
    const legacy = rule as EnvironmentHostGroupRule & { environment?: string };
    const envId = legacy.environmentId ?? legacy.environment ?? BUILTIN_ENVIRONMENT_IDS.OTHER;
    if (envId === BUILTIN_ENVIRONMENT_IDS.OTHER || envId === BUILTIN_ENVIRONMENT_IDS.HIDE) {
      continue;
    }
    if (!customIds.has(envId)) {
      customIds.add(envId);
      customEnvs.push({
        id: envId,
        label: LEGACY_ENV_LABELS[envId] ?? envId,
        builtin: false,
        hidden: false,
      });
    }
  ...
  const environments = normalizeEnvironments(customEnvs);
  const rules = sanitizeEnvironmentRules(
    normalizeEnvironmentRules(stored.environmentHostGroupRules),
    environments,
  );
